...
class ButterscotchSignature:
    ...
    def freq_centerpoints(self):
        if self.linear_bands:
            ...
            return audioprocessing.util.log2_average(
                numpy.linspace(0, self.highest_freq, 2 ** (len(self.bands) - 1) + 1)[1:])

    def __str__(self):
        f = self.freq_centerpoints()
        startfreq = f[0]
        endfreq = f[-1]
        if self.linear_intensities:
            u = "raw: 0 dB = value 1.0"
        else:
            u = "measured in dB"
        if self.linear_bands:
            l = "linear"
        else:
            l = "logarithmically averaged"
        return "Butterscotch signature containing %d %d-second blocks of %d %s bands (%s), whose centers range from %d to %d Hz, starting at sample %d in the original audio data." % (
        len(self.blocks), self.secs_per_block, len(self.bands), l, u, startfreq, endfreq, self.audio_onset_sample)
    ...
    def __repr__(self):
        ...
        if self.linear_bands:
            ...
            l = "log"
        return "<Butterscotch: %s blocks, %d s/block, %d %s bands, %s, %d Hz, onset %d>" % (
        len(self.blocks), self.secs_per_block, len(self.bands), l, u, self.highest_freq, self.audio_onset_sample)
    ...
    def halve_highest_freq(self):
        if self.linear_bands:
            if divmod(len(self.bands), 2)[
                1] != 0: raise ValueError, "Number of bands in original signature is odd, cannot halve odd numbers."
    ...
    def halve_block_count(self):
        if divmod(len(self.blocks), 2)[
            1] != 0: raise ValueError, "Number of blocks in original signature is odd, cannot halve odd numbers."
...
%s""" % ("%prog", cmdline, description)
    ...
    parser.add_option("-d", "--decibel",
                      help="Compute decibel (dB) levels instead of raw signal levels (the default).  Useful for plotting spectrums.",
                      action="store_true", dest="use_dB")
    parser.add_option("-l", "--linear-bands",
                      help="Leave the frequency bands linear instead of logarithmically averaging them (the default).  Useful for plotting spectrums.",
                      action="store_true", dest="use_linear_bands")
    parser.add_option("-s", "--full-spectrum",
                      help="Compute the full spectrum below the Nyquist frequency instead of discarding the high half (the default).  Useful to plot the full spectrum, and to compare high frequency loss in lossly encoded files.",
                      action="store_true", dest="use_full_spectrum")

    parser.add_option("-e", "--seconds-per-block", help="Change the number of seconds per block from the default 4.",
                      dest="spb", type="int", default=4)
    parser.add_option("-n", "--num-blocks", help="Change the number of blocks from the default 30.", dest="blocks",
                      type="int", default=30)
    parser.add_option("-b", "--num-bands",
                      help="Change the number of frequency bands from the default 256.  This always refer to linear bands, even when --log-bands is specified, and if --log-bands is specified, --num-bands must be a power of two.  Unless --full-spectrum is also specified, the resulting bands will be the lower half and the number of bands will also be half.",
                      dest="bands", type="int", default=256)
