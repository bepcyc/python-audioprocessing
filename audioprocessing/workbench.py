...
def plot_butterscotch_signature(self, title=None, show=True):
    ...
    if self.linear_bands:
        ax.set_ylabel('Frequency (%d linear bands)' % len(self.bands))
    else:
        ax.set_ylabel('Frequency (%d logarithmic bands)' % len(self.bands))
    if self.linear_intensities:
        ax.set_zlabel('Intensity (raw)')
    else:
...
def corr_two(one, two):
    global corrcache
    if (one, two) in corrcache:
        result = corrcache[(one, two)]
    elif (two, one) in corrcache:
        result = corrcache[(two, one)]
    else:
...
def mpg321(args, skip=0, frames=40):
    ...
    for a in args:
        print a
        subprocess.check_call(["mpg321", "-k", str(skip), "-n", str(frames), "-o", "alsa", a],
